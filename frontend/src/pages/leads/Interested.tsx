// src/pages/leads/InterestedLeads.tsx
import { useState, useEffect, useCallback } from "react";
import {
  Search, Trash2, Download, Phone, MessageCircle,
  Edit3, X, Calendar, Clock, Globe, Zap, Target, UserCheck,
} from "lucide-react";
...
const BULK_STATUS_OPTIONS = [
  { value: "Converted", label: "🎓 Finalized (Won)"        },
  { value: "Follow-up", label: "⏳ Move to Follow-up"      },
  { value: "Lost",      label: "❌ Mark as Lost"            },
  { value: "Contacted", label: "⏮️ Move back to Contacted" },
];
...
export default function InterestedLeads() {
  ...
  const handleExport = () => {
    const headers = ["ID", "Name", "Phone", "City", "Course", "Quality", "Source", "Updated", "Assigned", "Notes"];
    const csv = [
      headers.join(","),
      ...leads.map((l) => [
    ...
    const link = Object.assign(document.createElement("a"), {
      ...
      download: `interested_leads_${new Date().toISOString().split("T")[0]}.csv`,
    });
    ...
    <div className="space-y-4 pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-gray-900 dark:text-white tracking-tight flex items-center gap-2.5">
            <span className="w-8 h-8 bg-violet-600 rounded-xl flex items-center justify-center shadow-lg shadow-violet-600/20 shrink-0">
              <Target size={16} className="text-white" />
            </span>
            Interested Repository
          </h1>
          <p className="text-[10px] font-black text-violet-600 uppercase tracking-widest mt-1 flex items-center gap-2">
            High Intent Pipeline
            {hasActiveFilters && <span className="text-orange-500">· Filters active</span>}
          </p>
        </div>
        <button type="button" onClick={handleExport}
          className="flex items-center gap-2 px-4 py-2 bg-gray-900 dark:bg-gray-700 text-white rounded-xl text-[10px] font-black tracking-widest hover:bg-violet-600 transition-all shadow-lg self-start shrink-0">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm p-3 sm:p-2">
        <div className="flex flex-wrap items-center gap-3">
            ...
            {(["assign", "edit", "delete"] as BulkMode[]).map((m) => (
              <button key={m} type="button"
                ...
                className={`px-3 sm:px-4 py-2 rounded-lg transition-all capitalize ${
                  bulkMode === m
                    ? `bg-white dark:bg-gray-700 shadow-sm font-black ${m === "assign" ? "text-violet-600" : m === "edit" ? "text-amber-600" : "text-red-600"}`
                    : "text-gray-500"
                }`}>
          ...
          {selectedLeads.length > 0 && (
            <span className="text-[10px] font-black text-violet-600 bg-violet-50 dark:bg-violet-900/20 px-2.5 py-1.5 rounded-full border border-violet-100 dark:border-violet-800 shrink-0">
          ...
          {isAdmin && bulkMode === "assign" && selectedLeads.length > 0 && (
            <div className="flex items-center gap-2 bg-violet-50 dark:bg-violet-900/20 px-3 py-2 rounded-xl border border-violet-100 dark:border-violet-800">
              ...
              <select value={targetCounselorId} onChange={(e) => setTargetCounselorId(e.target.value)}
                className="bg-transparent text-xs font-black text-violet-700 dark:text-violet-300 outline-none min-w-[140px] cursor-pointer">
              ...
              <button type="button" onClick={handleBulkAssign}
                ...
                className="bg-violet-600 hover:bg-violet-700 text-white px-4 py-1.5 rounded-lg font-black text-[10px] uppercase tracking-widest disabled:opacity-50 transition-all active:scale-95 shrink-0">
            ...
            <div className="flex flex-wrap gap-1.5">
              {RANGE_BTNS.map((r) => (
                <button key={r.id} type="button"
                  ...
                  <span className={activeRangeId === r.id ? "text-white" : "text-violet-500"}>{r.icon}</span>
                  {r.label}
                </button>
              ))}
            </div>
          ...
          <table className="w-full text-left border-collapse min-w-[1020px]">
            <thead className="bg-gray-50 dark:bg-gray-800/60 border-b border-gray-100 dark:border-gray-800">
              <tr className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                <th className="px-3 py-4 w-8 text-center"><input type="checkbox" checked={allSelected} onChange={toggleSelectAll} className="w-3.5 h-3.5 accent-violet-600" aria-label="Select all" /></th>
                ...
                {isAdmin && <th className="px-4 py-4 text-violet-600">Assigned To</th>}
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {loading ? (
                <tr>
                  <td colSpan={isAdmin ? 15 : 14} className="py-20 text-center">
                    <div className="flex flex-col items-center gap-2 text-gray-400">
                      <div className="w-5 h-5 border-4 border-violet-600 border-t-transparent rounded-full animate-spin" />
                      <span className="text-[10px] font-black uppercase tracking-widest">Syncing…</span>
                    </div>
                  </td>
                </tr>
              ) : leads.length === 0 ? (
                <tr><td colSpan={isAdmin ? 15 : 14} className="py-20 text-center text-gray-400 font-bold uppercase tracking-widest text-xs">📭 No interested leads found</td></tr>
              ) : leads.map((lead, index) => (
                <tr key={lead.id} className={`text-sm transition-colors hover:bg-gray-50/80 dark:hover:bg-gray-800/30 ${
                  selectedLeads.includes(lead.id) ? "bg-violet-50/30 dark:bg-violet-900/10" : ""
                }`}>
                  <td className="px-3 py-1.5 text-center align-middle"><input type="checkbox" checked={selectedLeads.includes(lead.id)} onChange={() => toggleSelect(lead.id)} className="w-3.5 h-3.5 accent-violet-600" /></td>
                  ...
                  <td className="px-4 py-1.5 whitespace-nowrap align-middle">
                    <p className="text-[10px] font-bold text-violet-700 dark:text-violet-400">{fmtDate(lead.updated_at)}</p>
                    <p className="text-[8px] text-violet-500/60 uppercase font-medium tracking-tighter">Updated</p>
                  </td>
                  {isAdmin && (
                    <td className="px-3 py-1.5 align-middle min-w-[130px]">
                      <select value={lead.assigned_user_id || ""} onChange={(e) => handleTransferLead(lead.id, e.target.value)}
                        className="w-full h-7 text-[9px] font-black text-violet-600 bg-violet-50 dark:bg-violet-900/20 px-1.5 rounded-lg uppercase border border-violet-100 dark:border-violet-800 outline-none cursor-pointer">
                  ...
                  <td className="px-4 py-1.5 text-right align-middle whitespace-nowrap">
                    <div className="flex justify-end items-center gap-2">
                      <button type="button" onClick={() => openEdit(lead)} className="text-violet-500 hover:bg-violet-50 p-1.5 rounded-lg transition-all" aria-label="Edit"><Edit3 size={14} /></button>
      ...
      <div className="sm:hidden space-y-3">
        {leads.length > 0 && (
          <div className="flex items-center justify-between px-1">
            <label className="flex items-center gap-2 text-xs font-bold text-gray-600 dark:text-gray-400 cursor-pointer">
              <input type="checkbox" checked={allSelected} onChange={toggleSelectAll} className="w-4 h-4 accent-violet-600 rounded" />
              Select All ({leads.length})
            </label>
            {selectedLeads.length > 0 && (
              <span className="text-[10px] font-black text-violet-600 bg-violet-50 dark:bg-violet-900/20 px-2.5 py-1 rounded-full border border-violet-100">
                {selectedLeads.length} selected
              </span>
            )}
          </div>
        ...
        {loading ? (
          ...
          <div className="py-16 text-center text-gray-400 text-sm">📭 No interested leads found</div>
        ) : (
          <>
            {leads.map((lead, index) => (
              <div key={lead.id} className={`bg-white dark:bg-gray-900 rounded-2xl border shadow-sm overflow-hidden ${
                selectedLeads.includes(lead.id) ? "border-violet-300 ring-1 ring-violet-200 dark:border-violet-700" : "border-gray-200 dark:border-gray-800"
              }`}>
                <div className="flex items-start gap-2.5 p-3 border-b border-gray-100 dark:border-gray-800">
                  <input type="checkbox" checked={selectedLeads.includes(lead.id)} onChange={() => toggleSelect(lead.id)} className="w-4 h-4 accent-violet-600 rounded mt-0.5 shrink-0" />
                  <div className="w-7 h-7 rounded-full bg-violet-100 dark:bg-violet-900/40 flex items-center justify-center text-violet-700 font-black text-[10px] shrink-0">
                    {(currentPage - 1) * rowsPerPage + index + 1}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-gray-900 dark:text-white text-sm truncate uppercase">{lead.full_name}</p>
                    <p className="text-[10px] text-violet-600 font-medium uppercase">
                      {lead.qualification || "—"}{lead.year_of_passing ? ` · ${lead.year_of_passing}` : ""}
                    </p>
                  </div>
                  <div className="shrink-0">
                    {lead.assigned_user_name ? (
                      <span className="text-[9px] font-black text-violet-600 bg-violet-50 px-2 py-1 rounded-full border border-violet-100 uppercase">{lead.assigned_user_name}</span>
                    ) : (
                ...
                <div className="p-3 grid grid-cols-2 gap-x-4 gap-y-2">
                  ...
                  <div><p className="text-[9px] font-black uppercase tracking-widest text-gray-400">Updated</p><p className="text-xs font-bold text-violet-600">{fmtDate(lead.updated_at)}</p></div>
                ...
                {isAdmin && (
                  <div className="px-3 py-2 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-100 dark:border-gray-800">
                    <select value={lead.assigned_user_id || ""} onChange={(e) => handleTransferLead(lead.id, e.target.value)}
                      className="w-full text-xs font-black text-violet-600 bg-violet-50 dark:bg-violet-900/20 px-3 py-2 rounded-xl border border-violet-100 dark:border-violet-800 outline-none">
                ...
                <div className="flex items-center justify-between px-3 py-2 bg-gray-50 dark:bg-gray-800/60 border-t border-gray-100 dark:border-gray-800">
                  ...
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={() => openEdit(lead)} className="p-1.5 text-violet-500 hover:bg-violet-50 rounded-xl" aria-label="Edit"><Edit3 size={14} /></button>
      ...
      <DeleteModal
        ...
        message={showBulkDeleteModal
          ? `This will permanently remove all ${selectedLeads.length} selected interested leads. This cannot be undone.`
