...
import {
  Search, Trash2, Download, Phone, MessageCircle, Edit3,
...
export default function NewLeads() {
  ...
  // ── Load master data (ONE effect, counselors filtered to role=counselor) ──
    ...
    const fetchMasters = async () => {
      ...
      if (usersRes.status === "fulfilled") {
        const all = Array.isArray(usersRes.value)
          ? usersRes.value
          : (usersRes.value?.data ?? []);
        // Only counselors appear in the filter + assign dropdowns
  ...
  return (
    <div className="space-y-4 pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
        <div>
          ...
          <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest mt-1 flex items-center gap-2">
            Unassigned Queue · Incoming Stream
            {hasActiveFilters && <span className="text-orange-500">· Filters active</span>}
          </p>
        </div>
        <button type="button" onClick={handleExport}
          className="flex items-center gap-2 px-4 py-2 bg-gray-900 dark:bg-gray-700 text-white rounded-xl text-[10px] font-black tracking-widest hover:bg-blue-600 transition-all shadow-lg self-start shrink-0">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm p-3 sm:p-2">
        <div className="flex flex-wrap items-center gap-3">
            ...
            {(["assign", "edit", "delete"] as BulkMode[]).map((m) => (
              <button key={m} type="button"
                ...
                className={`px-3 sm:px-4 py-2 rounded-lg transition-all capitalize ${
                  bulkMode === m
                    ? `bg-white dark:bg-gray-700 shadow-sm font-black ${m === "assign" ? "text-blue-600" : m === "edit" ? "text-amber-600" : "text-red-600"}`
                    : "text-gray-500"
                }`}>
          ...
          {selectedLeads.length > 0 && (
            <span className="text-[10px] font-black text-blue-600 bg-blue-50 dark:bg-blue-900/20 px-2.5 py-1.5 rounded-full border border-blue-100 dark:border-blue-800 shrink-0">
          ...
          {isAdmin && bulkMode === "assign" && selectedLeads.length > 0 && (
            <div className="flex items-center gap-2 bg-blue-50 dark:bg-blue-900/20 px-3 py-2 rounded-xl border border-blue-100 dark:border-blue-800">
              ...
              <select value={targetCounselorId} onChange={(e) => setTargetCounselorId(e.target.value)}
                className="bg-transparent text-xs font-black text-blue-700 dark:text-blue-300 outline-none min-w-[140px] cursor-pointer">
              ...
              <button type="button" onClick={handleBulkAssign}
                ...
                className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-1.5 rounded-lg font-black text-[10px] uppercase tracking-widest disabled:opacity-50 transition-all active:scale-95 shrink-0">
      ...
      <div className="bg-white -mt-3 dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm p-3 sm:p-2">
        <div className="flex flex-col lg:flex-row gap-3">
          <div className="relative w-full sm:max-w-xs"> 
            ...
            <input 
              type="text" 
              placeholder="Search new leads by name or phone…"
              value={filters.search} 
              onChange={(e) => updateFilters({ search: e.target.value })}
              className="w-full pl-9 pr-4 py-2 bg-gray-50 dark:bg-gray-800 dark:text-white border border-gray-200 dark:border-gray-700 rounded-xl text-sm outline-none focus:border-blue-400 transition-all" 
            />
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[820px]">
            <thead className="bg-gray-50 dark:bg-gray-800/60 border-b border-gray-100 dark:border-gray-800">
              <tr className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                <th className="px-3 py-4 w-8 text-center">
                  <input type="checkbox" checked={allSelected} onChange={toggleSelectAll} className="w-3.5 h-3.5 accent-blue-600" aria-label="Select all" />
                </th>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {loading ? (
                <tr><td colSpan={isAdmin ? 11 : 10} className="py-20 text-center text-gray-400 font-bold uppercase tracking-widest text-xs">📡 Loading new leads…</td></tr>
              ) : leads.length === 0 ? (
                <tr><td colSpan={isAdmin ? 11 : 10} className="py-20 text-center text-gray-400 font-bold uppercase tracking-widest text-xs">📭 No new leads found</td></tr>
              ) : leads.map((lead, index) => (
                <tr key={lead.id} className={`text-sm transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/30 ${
                  selectedLeads.includes(lead.id) ? "bg-blue-50/40 dark:bg-blue-900/10" : ""
                }`}>
                  <td className="px-3 py-2 text-center align-middle">
                    <input type="checkbox" checked={selectedLeads.includes(lead.id)} onChange={() => toggleSelect(lead.id)} className="w-3.5 h-3.5 accent-blue-600" />
                  </td>
                  <td className="px-2 py-2 text-[11px] text-gray-400 text-center align-middle tabular-nums">
                  ...
                  {isAdmin && (
                    <td className="px-3 py-2 align-middle min-w-[130px]">
                      <select value={lead.assigned_user_id || ""} onChange={(e) => handleTransferLead(lead.id, e.target.value)}
                        className="w-full h-7 text-[9px] font-black text-blue-600 bg-blue-50 dark:bg-blue-900/20 px-1.5 rounded-lg uppercase border border-blue-100 dark:border-blue-800 outline-none cursor-pointer">
                  ...
                  <td className="px-4 py-2 text-right align-middle whitespace-nowrap">
                    <div className="flex justify-end items-center gap-2">
                      <button type="button" onClick={() => openEdit(lead)} className="text-blue-500 hover:bg-blue-50 p-1.5 rounded-lg transition-all" aria-label="Edit"><Edit3 size={14} /></button>
                      ...
                      {isAdmin && (
                        <button type="button" onClick={() => setDeleteId(lead.id)} className="text-gray-300 hover:text-red-500 hover:bg-red-50 p-1.5 rounded-lg transition-all" aria-label="Delete"><Trash2 size={13} /></button>
                      )}
                    </div>
                  </td>
                </tr>
      ...
      <div className="sm:hidden space-y-3">
        {leads.length > 0 && (
          <div className="flex items-center justify-between px-1">
            <label className="flex items-center gap-2 text-xs font-bold text-gray-600 dark:text-gray-400 cursor-pointer">
              <input type="checkbox" checked={allSelected} onChange={toggleSelectAll} className="w-4 h-4 accent-blue-600 rounded" />
              Select All ({leads.length})
            </label>
            {selectedLeads.length > 0 && (
              <span className="text-[10px] font-black text-blue-600 bg-blue-50 dark:bg-blue-900/20 px-2.5 py-1 rounded-full border border-blue-100 dark:border-blue-800">
                {selectedLeads.length} selected
              </span>
            )}
          </div>
        ...
        {loading ? (
          <div className="py-16 text-center text-gray-400 text-sm">📡 Loading new leads…</div>
        ) : leads.length === 0 ? (
          <div className="py-16 text-center text-gray-400 text-sm">📭 No new leads found</div>
        ) : (
          <>
            {leads.map((lead) => (
              <div key={lead.id} className={`bg-white dark:bg-gray-900 rounded-2xl border shadow-sm overflow-hidden transition-all ${
                selectedLeads.includes(lead.id) ? "border-blue-300 ring-1 ring-blue-200 dark:border-blue-700" : "border-gray-200 dark:border-gray-800"
              }`}>
                <div className="flex items-start gap-2.5 p-3 border-b border-gray-100 dark:border-gray-800">
                  <input type="checkbox" checked={selectedLeads.includes(lead.id)} onChange={() => toggleSelect(lead.id)} className="w-4 h-4 accent-blue-600 rounded mt-0.5 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-gray-900 dark:text-white text-sm uppercase truncate">{lead.full_name}</p>
                  ...
                  <div className="shrink-0">
                    {lead.assigned_user_name ? (
                      <span className="text-[9px] font-black text-blue-600 bg-blue-50 dark:bg-blue-900/20 px-2 py-1 rounded-full border border-blue-100 uppercase">{lead.assigned_user_name}</span>
                    ) : (
                      <span className="text-[9px] font-black text-orange-500 bg-orange-50 dark:bg-orange-900/20 px-2 py-1 rounded-full border border-orange-100 uppercase">Waiting</span>
                    )}
                  </div>
                </div>
                <div className="p-3 grid grid-cols-2 gap-x-4 gap-y-2">
                  ...
                  <div><p className="text-[9px] font-black uppercase tracking-widest text-gray-400">Email</p><p className="text-xs text-gray-500 lowercase truncate">{lead.email || "—"}</p></div>
                </div>
                {isAdmin && (
                  <div className="px-3 py-2 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-100 dark:border-gray-800">
                    <select value={lead.assigned_user_id || ""} onChange={(e) => handleTransferLead(lead.id, e.target.value)}
                      className="w-full text-xs font-black text-blue-600 bg-blue-50 dark:bg-blue-900/20 px-3 py-2 rounded-xl border border-blue-100 dark:border-blue-800 outline-none">
                ...
                <div className="flex items-center justify-between px-3 py-2 bg-gray-50 dark:bg-gray-800/60 border-t border-gray-100 dark:border-gray-800">
                  <div className="flex items-center gap-3">
                    ...
                    <a href={`https://wa.me/91${lead.phone?.replace(/\D/g, "")}`} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 text-[11px] font-bold text-green-600"><MessageCircle size={12} /> WhatsApp</a>
                  </div>
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={() => openEdit(lead)} className="p-1.5 text-blue-500 hover:bg-blue-50 rounded-xl" aria-label="Edit"><Edit3 size={14} /></button>
      ...
      <DeleteModal
        ...
        message={showBulkDeleteModal
          ? `This will permanently remove all ${selectedLeads.length} selected records. This action cannot be undone.`
