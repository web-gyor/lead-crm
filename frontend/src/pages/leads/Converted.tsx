// src/pages/leads/ConvertedLeads.tsx
import { useState, useEffect, useCallback } from "react";
import {
  Search, Trash2, Download, Phone, MessageCircle,
  Edit3, X, Calendar, Clock, Globe, Zap, Award, UserCheck,
} from "lucide-react";
...
const BULK_STATUS_OPTIONS = [
  ...
  { value: "Follow-up",  label: "⏳ Back to Follow-up"       },
  { value: "Lost",       label: "❌ Mark as Lost (Cancelled)" },
];
  ...
  useEffect(() => {
    const fetchMasters = async () => {
      ...
      if (usersRes.status === "fulfilled") {
        const all = Array.isArray(usersRes.value) ? usersRes.value : (usersRes.value?.data ?? []);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const params: Record<string, string> = {
        ...
        status: LEAD_STATUS,        // ← "Converted" — not "Interested"
      };
  ...
  return (
    <div className="space-y-4 pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-gray-900 dark:text-white tracking-tight flex items-center gap-2.5">
            <span className="w-8 h-8 bg-green-600 rounded-xl flex items-center justify-center shadow-lg shadow-green-600/20 shrink-0">
              <Award size={16} className="text-white" />
            </span>
            Converted Repository
          </h1>
          <p className="text-[10px] font-black text-green-600 uppercase tracking-widest mt-1 flex items-center gap-2">
            Success Pipeline · Won Students
            {hasActiveFilters && <span className="text-orange-500">· Filters active</span>}
          </p>
        </div>
        <button type="button" onClick={handleExport}
          className="flex items-center gap-2 px-4 py-2 bg-gray-900 dark:bg-gray-700 text-white rounded-xl text-[10px] font-black tracking-widest hover:bg-green-600 transition-all shadow-lg self-start shrink-0">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm p-3 sm:p-2">
        <div className="flex flex-wrap items-center gap-3">
            ...
            {(["assign", "edit", "delete"] as BulkMode[]).map((m) => (
              <button key={m} type="button"
                ...
                className={`px-3 sm:px-4 py-2 rounded-lg transition-all capitalize ${
                  bulkMode === m
                    ? `bg-white dark:bg-gray-700 shadow-sm font-black ${m === "assign" ? "text-green-600" : m === "edit" ? "text-amber-600" : "text-red-600"}`
                    : "text-gray-500"
                }`}>
          ...
          {selectedLeads.length > 0 && (
            <span className="text-[10px] font-black text-green-700 bg-green-50 dark:bg-green-900/20 px-2.5 py-1.5 rounded-full border border-green-200 dark:border-green-800 shrink-0">
          ...
          {isAdmin && bulkMode === "assign" && selectedLeads.length > 0 && (
            <div className="flex items-center gap-2 bg-green-50 dark:bg-green-900/20 px-3 py-2 rounded-xl border border-green-100 dark:border-green-800">
              <UserCheck size={14} className="text-green-600 shrink-0" />
              <select value={targetCounselorId} onChange={(e) => setTargetCounselorId(e.target.value)}
                className="bg-transparent text-xs font-black text-green-700 dark:text-green-300 outline-none min-w-[140px] cursor-pointer">
              ...
              <button type="button" onClick={handleBulkAssign}
                ...
                className="bg-green-600 hover:bg-green-700 text-white px-4 py-1.5 rounded-lg font-black text-[10px] uppercase tracking-widest disabled:opacity-50 transition-all active:scale-95 shrink-0">
          ...
          {isAdmin && bulkMode === "edit" && selectedLeads.length > 0 && (
            <div className="flex flex-wrap items-center gap-2 bg-amber-50 dark:bg-amber-900/10 px-3 py-2 rounded-xl border border-amber-100 dark:border-amber-800">
              <select value={bulkSourceId} onChange={(e) => setBulkSourceId(e.target.value)}
                ...
                <option value="">Correct Source…</option>
              ...
              <select value={bulkStatus} onChange={(e) => setBulkStatus(e.target.value)}
                ...
                <option value="">Rollback Status…</option>
      ...
      <div className="bg-white -mt-3 dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm p-3 sm:p-2">
        <div className="flex flex-col lg:flex-row gap-3">
          <div className="relative w-full sm:max-w-xs xl:max-w-[240px] flex-shrink-0">
            ...
            <input type="text" placeholder="Search won students…"
              value={filters.search} onChange={(e) => updateFilters({ search: e.target.value })}
              className="w-full pl-9 pr-4 py-2 bg-gray-50 dark:bg-gray-800 dark:text-white border border-gray-200 dark:border-gray-700 rounded-xl text-sm outline-none focus:border-green-400 transition-all" />
          </div>
          ...
          <div className="flex flex-wrap xl:flex-nowrap items-center gap-2 w-full">
            ...
            <div className="flex flex-wrap gap-1.5">
              {RANGE_BTNS.map((r) => (
                <button key={r.id} type="button"
                  ...
                  <span className={activeRangeId === r.id ? "text-white" : "text-green-500"}>{r.icon}</span>
                  {r.label}
                </button>
              ))}
            </div>
            ...
            <div className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border flex-shrink-0 transition-all ${
              filters.startDate || filters.endDate ? "bg-green-50 border-green-300" : "bg-gray-50 border-gray-200 dark:bg-gray-800 dark:border-gray-700"
            }`}>
      ...
      {/* ════════════════════════════════════
          DESKTOP TABLE
      ════════════════════════════════════ */}
      <div className="hidden -mt-2 sm:block bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden">
          ...
          <table className="w-full text-left border-collapse min-w-[980px]">
            <thead className="bg-gray-50 dark:bg-gray-800/60 border-b border-gray-100 dark:border-gray-800">
              <tr className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                <th className="px-3 py-4 w-8 text-center">
                  <input type="checkbox" checked={allSelected} onChange={toggleSelectAll} className="w-3.5 h-3.5 accent-green-600" aria-label="Select all" />
                </th>
                ...
                {isAdmin && <th className="px-4 py-4 text-green-600">Assigned To</th>}
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {loading ? (
                ...
                <tr><td colSpan={isAdmin ? 14 : 13} className="py-20 text-center text-gray-400 font-bold uppercase tracking-widest text-xs">📭 No converted leads found</td></tr>
              ) : leads.map((lead, index) => (
                <tr key={lead.id} className={`text-sm transition-colors hover:bg-gray-50/80 dark:hover:bg-gray-800/30 ${
                  selectedLeads.includes(lead.id) ? "bg-green-50/30 dark:bg-green-900/10" : ""
                }`}>
                  <td className="px-3 py-1.5 text-center align-middle">
                    <input type="checkbox" checked={selectedLeads.includes(lead.id)} onChange={() => toggleSelect(lead.id)} className="w-3.5 h-3.5 accent-green-600" />
                  </td>
                  <td className="px-2 py-1.5 text-[11px] text-gray-400 text-center align-middle tabular-nums">
                  ...
                  <td className="px-4 py-1.5 font-bold text-gray-800 dark:text-white align-middle truncate max-w-[130px]">{lead.full_name}</td>
                  ...
                  <td className="px-4 py-1.5 tabular-nums font-bold text-gray-700 dark:text-gray-200 text-[13px] align-middle whitespace-nowrap">
                  ...
                  {isAdmin && (
                    <td className="px-3 py-1.5 align-middle min-w-[130px]">
                      <select value={lead.assigned_user_id || ""} onChange={(e) => handleTransferLead(lead.id, e.target.value)}
                        className="w-full h-7 text-[9px] font-black text-green-700 bg-green-50 dark:bg-green-900/20 px-1.5 rounded-lg uppercase border border-green-100 dark:border-green-800 outline-none cursor-pointer">
                  ...
                  <td className="px-4 py-1.5 text-right align-middle whitespace-nowrap">
                    <div className="flex justify-end items-center gap-2">
                      <button type="button" onClick={() => openEdit(lead)} className="text-green-500 hover:bg-green-50 p-1.5 rounded-lg transition-all" aria-label="Edit"><Edit3 size={14} /></button>
      ...
      <div className="sm:hidden space-y-3">
        {leads.length > 0 && (
          <div className="flex items-center justify-between px-1">
            <label className="flex items-center gap-2 text-xs font-bold text-gray-600 dark:text-gray-400 cursor-pointer">
              <input type="checkbox" checked={allSelected} onChange={toggleSelectAll} className="w-4 h-4 accent-green-600 rounded" />
              Select All ({leads.length})
            </label>
            {selectedLeads.length > 0 && (
              <span className="text-[10px] font-black text-green-700 bg-green-50 dark:bg-green-900/20 px-2.5 py-1 rounded-full border border-green-200">
                {selectedLeads.length} selected
              </span>
            )}
          </div>
        ...
        {loading ? (
          <div className="py-16 text-center text-gray-400 text-sm">📡 Syncing…</div>
        ) : leads.length === 0 ? (
          <div className="py-16 text-center text-gray-400 text-sm">📭 No converted leads found</div>
        ) : (
          <>
            {leads.map((lead, index) => (
              <div key={lead.id} className={`bg-white dark:bg-gray-900 rounded-2xl border shadow-sm overflow-hidden ${
                selectedLeads.includes(lead.id) ? "border-green-300 ring-1 ring-green-200 dark:border-green-700" : "border-gray-200 dark:border-gray-800"
              }`}>
                {/* Header */}
                <div className="flex items-start gap-2.5 p-3 border-b border-gray-100 dark:border-gray-800">
                  <input type="checkbox" checked={selectedLeads.includes(lead.id)} onChange={() => toggleSelect(lead.id)} className="w-4 h-4 accent-green-600 rounded mt-0.5 shrink-0" />
                  <div className="w-7 h-7 rounded-full bg-green-100 dark:bg-green-900/40 flex items-center justify-center text-green-700 font-black text-[10px] shrink-0">
                    {(currentPage - 1) * rowsPerPage + index + 1}
                  </div>
                  ...
                  <div className="shrink-0">
                    {lead.assigned_user_name ? (
                      <span className="text-[9px] font-black text-green-700 bg-green-50 px-2 py-1 rounded-full border border-green-100 uppercase">{lead.assigned_user_name}</span>
                    ) : (
                      <span className="text-[9px] font-black text-orange-500 bg-orange-50 px-2 py-1 rounded-full border border-orange-100 uppercase">Unassigned</span>
                    )}
                  </div>
                </div>
                ...
                <div className="p-3 grid grid-cols-2 gap-x-4 gap-y-2">
                  ...
                  <div><p className="text-[9px] font-black uppercase tracking-widest text-gray-400">Course</p><p className="text-xs text-gray-600 dark:text-gray-400">{lead.interested_course || "General"}</p></div>
                  ...
                  <div><p className="text-[9px] font-black uppercase tracking-widest text-gray-400">Admission</p><p className="text-xs font-bold text-green-600">{fmtDate(lead.updated_at)}</p></div>
                ...
                {isAdmin && (
                  <div className="px-3 py-2 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-100 dark:border-gray-800">
                    <select value={lead.assigned_user_id || ""} onChange={(e) => handleTransferLead(lead.id, e.target.value)}
                      className="w-full text-xs font-black text-green-700 bg-green-50 dark:bg-green-900/20 px-3 py-2 rounded-xl border border-green-100 dark:border-green-800 outline-none">
                ...
                <div className="flex items-center justify-between px-3 py-2 bg-gray-50 dark:bg-gray-800/60 border-t border-gray-100 dark:border-gray-800">
                  ...
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={() => openEdit(lead)} className="p-1.5 text-green-500 hover:bg-green-50 rounded-xl" aria-label="Edit"><Edit3 size={14} /></button>
      ...
      <DeleteModal
        ...
        title={showBulkDeleteModal ? `Delete ${selectedLeads.length} Records?` : "Delete Converted Lead?"}
        message={showBulkDeleteModal
          ? `This will permanently remove all ${selectedLeads.length} selected records. This cannot be undone.`
          : "This will permanently remove this converted lead record. This cannot be undone."}
